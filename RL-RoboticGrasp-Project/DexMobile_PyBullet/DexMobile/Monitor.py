...
class Monitor(gym.Wrapper):
    ...
    def __init__(self,
        ...
        if filename is None:
            ...
            self.logger = csv.DictWriter(self.file_handler, fieldnames=('tid', 'r', 'l', 't', 's', 'i', 'a', 'g', 'f'))
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict[Any, Any]]:
        ...
        if done:
            ...
            affordance = self.env.affordance
            fail_reason = self.env.fail_reason
            task_id = self.env.task_id
            grasp = self.env.grasp
            ep_info = {"tid": task_id, "r": round(ep_rew, 6), "l": eplen, "t": round(time.time() - self.t_start, 6), "s": sus,
                       "i": index, "a": affordance, 'g': grasp, 'f': fail_reason}
